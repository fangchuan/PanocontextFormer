...
def physical_violation_loss(end_points, renderer_up, renderer_left ,image_size):
    ...
    for prefix in prefixes:
        ...
        physical_loss = torch.sum(inters_sum_up * final_mask + inters_sum_left * final_mask)/(torch.sum(final_mask) + 1e-6)/100
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = model.cuda()
    if(args.layout_flag):
        output_name = 'layoutmesh_detection_depth_2view_1210'
    else:
        output_name = 'detection_depth_2view_1210'
