...
def evaluate_one_time(test_loader, DATASET_CONFIG, CONFIG_DICT, AP_IOU_THRESHOLDS, model, criterion, args, time=0):
    ...
    for batch_idx, batch_data_label in enumerate(test_loader):
        ...
        end_points['point_clouds'] = end_points['point_clouds'].detach()
        if(args.layout_flag):
...
if __name__ == '__main__':
    ...
    opt = parse_option()
    opt.checkpoint_path = "log/layoutmesh_detection_depth_1view_1212_surp/igibson_1670829757/best_valid_epoch_350.pth"
    opt.avg_times = 1
