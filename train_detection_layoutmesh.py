    ...
    for batch_idx, batch_data_label in tqdm(enumerate(test_loader)):
        ...
        if config.layout_flag:
            ...
            if (batch_idx<5):
                ...
                save_obj(output_filepath+"_cross.obj", end_points['deep3d_meshes'][2].cpu().detach().verts_padded()[0,:,:], end_points['deep3d_meshes'][2].cpu().detach().faces_padded()[0,:,:])
        else:
            end_points['total_loss'] = end_points['object_loss']
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = model.cuda()
    if(args.layout_flag):
        output_name = 'layoutmesh_detection'
    else:
