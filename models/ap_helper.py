...
from utils.nms_2d_utils import get_bboxes_corners, get_plane_mask, binaryMaskIOU
...
def parse_predictions(end_points, config_dict, prefix="", size_cls_agnostic=False):
    ...
    for i in range(bsize):
        ...
        #     corners_3d = get_bboxes_corners(pred_heading_class[i, j], pred_heading_residual[i, j],
        #                                         pred_center[i, j, 0:3],
        #                                         pred_size_class[i, j], pred_size_residual[i, j])
        #     corners_3d_proposal.append(corners_3d)
        # corners_3d_batch_proposal.append(corners_3d_proposal)
    ...
    if False:
        for i in range(bsize):
            bbox_3d_batch = []
            bbox_3d_batch_obj_prob = []
            bbox_3d_batch_sem_cls = []
            index_back_up = []
            for j in range(pred_center.shape[1]):
                if pred_mask[i, j] == 1 and obj_prob[i, j] > config_dict['conf_thresh']:
                    bbox_3d_batch.append(get_plane_mask(corners_3d_batch_proposal[i][j]))
                    bbox_3d_batch_obj_prob.append(obj_prob[i, j])
                    bbox_3d_batch_sem_cls.append(pred_sem_cls[i, j])
                    index_back_up.append(j)

            bbox_combinations_list = [i for i in range(len(bbox_3d_batch))]
            bbox_combinations = list(combinations(bbox_combinations_list, 2))
            for bbox_combination in bbox_combinations:
                idx_1 = bbox_combination[0]
                idx_2 = bbox_combination[1]
                iou = binaryMaskIOU(bbox_3d_batch[idx_1], bbox_3d_batch[idx_2])
                if (iou>0.4):
                    if(bbox_3d_batch_sem_cls[idx_1] == bbox_3d_batch_sem_cls[idx_2]):
                        if(bbox_3d_batch_obj_prob[idx_1]>bbox_3d_batch_obj_prob[idx_2]):
                            pred_mask[i, [index_back_up[idx_2]]] = 0
                        else:
                            pred_mask[i, [index_back_up[idx_1]]] = 0
