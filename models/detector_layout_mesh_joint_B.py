...
from .transformer_custorm import TransformerDecoderLayer
...
class GroupFreeDetectorDeep3D(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        for i in range(self.num_decoder_layers):
            self.decoder.append(
                TransformerDecoderLayer(
                    ...
                    obj_posembed = self.decoder_self_posembeds[i],
                    pc_posembed = self.decoder_cross_posembeds[i],
                    layout_posembed=self.decoder_layout_posembeds[i],
                ))
        ...
        hidden_dim = 144
        self.layout_estimation_vert_offset = nn.Linear(hidden_dim +hidden_dim+ 3, 3)
        ...
        for i in range(6):
            if i == 0:
                ...
                input_dim = hidden_dim + vert_feat_dim + 3
            gconv = GraphConv(input_dim, hidden_dim*2, init="normal", directed=False)
    ...
    def forward(self, inputs):
        ...
        for i in range(self.num_decoder_layers):
            if(i>0):
                vert_feats_proj = torch.transpose(vert_feats_proj.view(bs, -1, 288), 1, 2)

            prefix = 'last_' if (i == self.num_decoder_layers - 1) else f'{i}head_'
            # transformer
            query, vert_feats_proj = self.decoder[i](query, key, vert_feats_proj, base_xyz, key_pos, vert_pos_packed.view(bs,-1,3))
            ...
            vert_align_feats = vert_feats_proj
        ...
        #
        # # Run graph conv layers
        # for gconv_idx, gconv in enumerate(self.layout_estimation_gconvs):
