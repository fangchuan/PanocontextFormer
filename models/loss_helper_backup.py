...
def compute_objectness_loss_based_on_query_points(end_points, num_decoder_layers):
    ...
    if num_decoder_layers > 0:
        prefixes = ['proposal_'] + ['last_'] + [f'{i}head_' for i in range(num_decoder_layers - 1)] + ['layout_cross']
    else:
...
def get_loss(end_points, config, num_decoder_layers,
    ...
    if (detection_flag):
        ...
        objectness_loss_sum = torch.tensor(0)
        end_points['sum_heads_objectness_loss'] = objectness_loss_sum
        box_loss_sum = torch.tensor(0)
        end_points['sum_heads_box_loss'] = box_loss_sum
        sem_cls_loss_sum = torch.tensor(0)
        end_points['sum_heads_sem_cls_loss'] = sem_cls_loss_sum
        emb_codes_loss = torch.tensor(0)
        end_points['emb_codes_loss'] = emb_codes_loss

    if(layout_flag):
        layout_total_loss = end_points['layout_total_loss_refine'] + end_points['layout_total_loss_initial']
    else:
        layout_total_loss = torch.tensor(0)
    # means average proposal with prediction loss
    loss = query_points_generator_loss_coef * query_points_generation_loss + \
           1.0 / (num_decoder_layers + 1) * (obj_loss_coef * objectness_loss_sum + box_loss_coef * box_loss_sum + sem_cls_loss_coef * sem_cls_loss_sum + emb_code_delta * emb_codes_loss) + \
           layout_total_loss / 10
    loss *= 10
