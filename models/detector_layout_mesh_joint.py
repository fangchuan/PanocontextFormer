...
class GroupFreeDetectorDeep3D(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        self.random_masking = is_training
        if(self.featrue_token_flag):
            self.decoder_global_featrue_proj = nn.Conv1d(512, 288, kernel_size=1)
            ...
            self.feat_pose_map = nn.Parameter(feat_pose_map.view(-1, 3).unsqueeze(0), requires_grad=False).to('cuda') # [4,512,3]
    ...
    def forward(self, inputs):
        ...
        if (self.random_masking):
            ...
            constant_tensor = torch.ones_like(query_joint[:,:,:meta_masks.shape[2]]) * 0.01
            query_joint[:,:,:meta_masks.shape[2]] = query_joint[:,:,:meta_masks.shape[2]] * meta_masks + constant_tensor * (1 - meta_masks)
        for i in range(self.num_decoder_layers):
            ...
            if (self.featrue_token_flag):
                feature_position_embs = self.decoder_feature_posembeds[i](self.feat_pose_map.repeat(bs,1,1))
