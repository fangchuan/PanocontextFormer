...
def compute_objectness_loss_based_on_query_points(end_points, num_decoder_layers):
    ...
    if num_decoder_layers > 0:
        if('layout_refine_objectness_scores' in end_points):
            prefixes = ['proposal_'] + ['last_'] + [f'{i}head_' for i in range(num_decoder_layers - 1)] + ['layout_refine_']
        else:
    ...
    for prefix in prefixes:
        ...
        # generate outside label
        seed_outside_gt = torch.gather(end_points['outside_label_mask'], 1, seed_inds)  # B,num_seed
        query_points_outside_gt = torch.gather(seed_outside_gt, 1, query_points_sample_inds)
        end_points[f'{prefix}outside_label'] = query_points_outside_gt
...
def compute_box_and_sem_cls_loss(end_points, config, num_decoder_layers,
    ...
    if num_decoder_layers > 0:
        if('layout_refine_center' in end_points):
            prefixes = ['proposal_'] + ['last_'] + [f'{i}head_' for i in range(num_decoder_layers - 1)] + ['layout_refine_']
        else:
            prefixes = ['proposal_'] + ['last_'] + [f'{i}head_' for i in range(num_decoder_layers - 1)]
    else:
    ...
    for prefix in prefixes:
        ...
        cls_sem_weight = False
        if(not cls_sem_weight):
            sem_cls_loss = torch.sum(sem_cls_loss * objectness_label) / (torch.sum(objectness_label) + 1e-6)
        else:
            sem_cls_weight = torch.gather(end_points['sem_cls_weight'], 1, object_assignment)  # select (B,K) from (B,K2)
            sem_cls_loss = torch.sum(sem_cls_loss * (objectness_label * sem_cls_weight)) / (torch.sum(objectness_label) + 1e-6)
        ...
        # 3.5 emb-code loss
        if (end_points[f'{prefix}emb_codes'] != None):
            emb_codes_dim = end_points['max_gt_latent_codes'].shape[2]
            num_object_assignment = object_assignment.shape[1]
            object_assignment_expand = object_assignment.view(batch_size, num_object_assignment, 1).repeat(1, 1, emb_codes_dim)
            latent_code_label = torch.gather(end_points['max_gt_latent_codes'], 1, object_assignment_expand)
            criterion_latent_code = nn.SmoothL1Loss(reduction='none')
            emb_codes_loss = criterion_latent_code(end_points[f'{prefix}emb_codes'], latent_code_label)
            emb_codes_loss = torch.sum(emb_codes_loss, dim=2)
            emb_codes_loss = torch.sum(emb_codes_loss * objectness_label) / (torch.sum(objectness_label) + 1e-6)
        else:
            emb_codes_loss = torch.tensor(0)
        end_points[f'{prefix}emb_codes_loss'] = emb_codes_loss

        if (end_points[f'{prefix}emb_codes'] != None):
            emb_codes_loss_sum += emb_codes_loss

    return box_loss_sum, sem_cls_loss_sum, end_points, emb_codes_loss_sum
...
def get_loss(end_points, config, num_decoder_layers,
    ...
    box_loss_sum, sem_cls_loss_sum, end_points, emb_codes_loss = compute_box_and_sem_cls_loss(
    ...
    end_points['emb_codes_loss'] = emb_codes_loss

    # means average proposal with prediction loss
    loss = query_points_generator_loss_coef * query_points_generation_loss + \
           1.0 / (num_decoder_layers + 2) * (obj_loss_coef * objectness_loss_sum + box_loss_coef * box_loss_sum + sem_cls_loss_coef * sem_cls_loss_sum + emb_code_delta * emb_codes_loss)
    loss *= 10

    end_points['object_loss'] = loss
