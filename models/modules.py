...
class PredictHead(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster,
                 mean_size_arr, num_proposal, seed_feat_dim=256, emb_codes_dim = 0):
        super().__init__()
        ...
        if self.emb_codes_dim>0:
            self.emb_codes_head = torch.nn.Conv1d(seed_feat_dim,self.emb_codes_dim,1)
    ...
    def forward(self, features, base_xyz, end_points, prefix=''):
        ...
        # emb code
        if self.emb_codes_dim > 0:
            emb_codes = self.emb_codes_head(net).transpose(2, 1)
        else:
            emb_codes = None
        ...
        end_points[f'{prefix}emb_codes'] = emb_codes
...
class ClsAgnosticPredictHead(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_proposal, seed_feat_dim=256, emb_codes_dim = 0):
        super().__init__()
