...
class GroupFreeDetectorDeep3D(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        if(self.layout_flag):
            ...
            self.layout_estimation_net = Deep3DlayoutNetFuse(backbone='resnet18', decoder_type='rcnn_p2m_mhsa_pos_dual', full_size=True, hidden_dim=288, fuse_type='biproj')
    ...
    def forward(self, inputs):
        ...
        end_points['last_object_featrue'] = query
        ...
        if self.layout_flag:
            ...
            # import numpy as np
            # np.save("./render_dump/attention_weight_layout.npy",end_points['attention_weight_layout'].detach().cpu().numpy())
            # np.save("./render_dump/attention_weight_object.npy",end_points['attention_weight_object'].detach().cpu().numpy())
            # np.save("./render_dump/object_pos.npy",object_pos.detach().cpu().numpy())
            # np.save("./render_dump/layout_pos.npy",vert_pos_padded.detach().cpu().numpy())
            # exit()
