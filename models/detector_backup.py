...
class GroupFreeDetector(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        if self.joint_flag:
            # 2个cross-attention，一个proposal 一个 oneconv
            self.joint_pred_bon = nn.Conv1d(288, 2, 1, padding=0, bias=False)
            self.joint_pred_cor = nn.Conv1d(288, 1, 1, padding=0, bias=False)
            self.joint_proposal = PredictHead(num_class, num_heading_bin, num_size_cluster,
                                              mean_size_arr, num_proposal, 288, emb_codes_dim=self.emb_codes_dim)

            self.joint_cross_layout =TransformerDecoderLayer(288, nhead, dim_feedforward, dropout, activation,
                                                              self_posembed=PositionEmbeddingLearned(6, 288),
                                                              cross_posembed=PositionEmbeddingLearned(6, 288),)

            self.joint_cross_obj = TransformerDecoderLayer(288, nhead, dim_feedforward, dropout, activation,
                                                              self_posembed=PositionEmbeddingLearned(6, 288),
                                                              cross_posembed=PositionEmbeddingLearned(6, 288),)
    ...
    def forward(self, inputs):
        ...
        if self.joint_flag:
            layout_featrue_proj = self.layout_feature_proj(end_points['layout_featrue']['1D'])
            object_featrue_proj = self.obj_featrue_proj(query)
            ...
            #cross attention
            layout_feature_update = self.joint_cross_layout(layout_featrue_proj, object_featrue_proj, layout_pos, object_pos)
            object_feature_update = self.joint_cross_obj(object_featrue_proj, layout_featrue_proj, object_pos, layout_pos)

            # Prediction
            base_xyz, base_size = self.joint_proposal(object_feature_update,
                                                      base_xyz=cluster_xyz,
                                                      end_points=end_points,
                                                      prefix='layout_cross')
            base_xyz = base_xyz.detach().clone()
