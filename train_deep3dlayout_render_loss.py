...
from igibson.generate_render_trainingdata import getTopViewRenderParms, generateMeshRenderImage
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, mesh_criterion, shape_criterion,
                    smooth_criterion, criterion_mesh_render, optimizer, scheduler, config):
    model.train()  # set model to training mode

    renderer = getTopViewRenderParms('cuda')
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        pred_mesh_render_image_0 = generateMeshRenderImage(end_points['deep3d_meshes'][0],renderer=renderer)
        pred_mesh_render_image_1 = generateMeshRenderImage(end_points['deep3d_meshes'][1], renderer=renderer)
        rener_loss_0 = criterion_mesh_render(pred_mesh_render_image_0, batch_data_label['gt_mesh_render_image'])
        rener_loss_1 = criterion_mesh_render(pred_mesh_render_image_1, batch_data_label['gt_mesh_render_image'])
        losses['rener_loss_0'] = rener_loss_0.item()
        losses['rener_loss_1'] = rener_loss_1.item()
        render_loss_coef = 1.0
        ...
        total_loss = mesh_loss + \
                     ...
                     (rener_loss_0 + rener_loss_1) / len(end_points['deep3d_meshes']) * render_loss_coef
...
if __name__ == '__main__':
    ...
    LOG_DIR = os.path.join(args.log_dir, 'deep3dlayout_render_loss', f'{args.dataset}_{int(time.time())}')

    args.log_dir = LOG_DIR
    os.makedirs(args.log_dir, exist_ok=True)

    logger = setup_logger(output=args.log_dir, name="deep3dlayout_render_loss")
    ...
    # criterion_mesh_render = torch.nn.CrossEntropyLoss(size_average=True)
    criterion_mesh_render = torch.nn.L1Loss()
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, mesh_criterion, sharp_criterion,
                        smooth_criterion, criterion_mesh_render, optimizer, scheduler, args)
