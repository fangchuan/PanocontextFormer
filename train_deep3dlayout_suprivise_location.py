...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, mesh_criterion, shape_criterion, smooth_criterion, vert_criterion, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        # mesh_loss, losses = mesh_criterion(meshes_pred = [end_points['deep3d_meshes'][1]], meshes_gt = gt_meshes)
        ...
        total_loss = mesh_loss + (sharp_loss_1) * config.dp3d_sharp_loss_coef + (smooth_loss_1) * config.dp3d_smooth_loss_coef + \
                     init_vert_loss* init_vert_loss_coef
...
if __name__ == '__main__':
    ...
    LOG_DIR = os.path.join(args.log_dir, 'deep3dlayout_suprivised_1211', f'{args.dataset}_{int(time.time())}')

    args.log_dir = LOG_DIR
    os.makedirs(args.log_dir, exist_ok=True)

    logger = setup_logger(output=args.log_dir, name="deep3dlayout_suprivised_1211")
