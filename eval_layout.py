...
from module.horizonnet.HorizonNet.dataset import cor_2_1d
from module.layout.misc import post_proc
import glob

def prepare_gtdt_pairs(gt_glob, dt_glob):
    gt_paths = sorted(glob.glob(gt_glob+"/*.txt"))
    dt_glob = sorted(glob.glob(dt_glob+"/*.txt"))
    dt_paths_txt = dict([(os.path.split(v)[-1].split('.')[0], v) for v in dt_glob])
    gtdt_pairs = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dt_glob',default='./layout_test_output',
                        type=str,
                        help='NOTE: Remeber to quote your glob path.'
                             'Files assumed to be json from inference.py')
    parser.add_argument('--gt_glob',default='/mnt/workspace/code/wallline_detection/data/igibson_layout/test/label_cor_occ',
                        type=str,
                        help='label_cor_occ label_cor')
