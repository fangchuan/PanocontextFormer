...
def parse_option():
    ...
    parser.add_argument('--detection_flag', default=False, type=bool, help='whether training 3d detection')
    ...
    parser.add_argument('--emb_code_delta', default=0.05, type = float, help='128: 0.05; 512: 0.00002')
    ...
    parser.add_argument('--decoder_learning_rate', type=float, default=0.0006,
    ...
    parser.add_argument('--val_freq', type=int, default=25, help='val frequency')

    # others
    parser.add_argument("--local_rank", type=int, help='local rank for DistributedDataParallel')
    parser.add_argument('--ap_iou_thresholds', type=float, default=[0.25], nargs='+',
...
def get_loader(args):
    ...
    if args.dataset == 'igibson':
        ...
        TRAIN_DATASET = IGbsonDetectionDataset('train', num_points=args.num_point,
                                                 ...
                                                 layout_dataset_flag=args.layout_flag)
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        loss, end_points = criterion(end_points, DATASET_CONFIG,
                                     ...
                                     detection_flag=config.detection_flag,
...
def evaluate_one_epoch(test_loader, DATASET_CONFIG, CONFIG_DICT, AP_IOU_THRESHOLDS, model, criterion, config):
    ...
    if config.num_decoder_layers > 0:
        prefixes = ['last_', 'proposal_'] + [f'{i}head_' for i in range(config.num_decoder_layers - 1)] + ['layout_cross']
    else:
    ...
    for batch_idx, batch_data_label in enumerate(test_loader):
        ...
        if(config.detection_flag == False):
            return 0,0
...
if __name__ == '__main__':
    ...
    logger = setup_logger(output=opt.log_dir, name="group-free")
