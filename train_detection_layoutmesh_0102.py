...
def get_merged_bs_meshes(bs_bbox_mesh, obj_ass_label, outside_label):
    ...
    for bs_idx in range(bs):
        ...
        for i in range(256):
            ...
            sub_bbox = bs_bbox_mesh[bbox_idx]
            if (obj_ass_label[bs_idx, i]<0.5 or outside_label[bs_idx, i]>0.5): continue
            merged_mesh_list.append(sub_bbox)
        merged_mesh = pytorch3d.structures.join_meshes_as_batch(merged_mesh_list)
        verts_rgb = torch.ones_like(merged_mesh.verts_packed())[None]  # (1, V, 3)
        textures = TexturesVertex(verts_rgb)
        merged_textured_mesh = Meshes(
            verts=[merged_mesh.verts_packed().to('cuda')],
            faces=[merged_mesh.faces_packed().to('cuda')],
            textures=textures.to('cuda')
        )
        return_meshes.append(merged_textured_mesh)
    output = pytorch3d.structures.join_meshes_as_batch(return_meshes)

    return output

def get_merged_bs_meshes2(bs_bbox_mesh, obj_ass_label, outside_label):

    bs = obj_ass_label.shape[0]
    return_meshes = []
    all_verts = bs_bbox_mesh.verts_padded()
    all_edges = bs_bbox_mesh.faces_padded()
    for bs_idx in range(bs):
        ...
        verts_rgb = torch.ones_like(merged_mesh_verts)[None]  # (1, V, 3)
        textures = TexturesVertex(verts_rgb)
        merged_textured_mesh = Meshes(
...
def physical_violation_loss(end_points, renderer ,image_size):
    ...
    renderer_layout_image_up = renderer(textured_mesh).unsqueeze(1)
    ...
    for prefix_idx, prefix in enumerate(prefixes):
        ...
        merged_batch_bbox_meshes = get_merged_bs_meshes2(batch_bbox_mesh, obj_ass_mask, obj_outside_label)
        
        renderer_bbox_image_up = renderer(merged_batch_bbox_meshes)
        
        renderer_bbox_image_up = renderer_bbox_image_up.view(bs,-1, image_size, image_size, 1)

        outside_bbox_image_up = (1 - renderer_layout_image_up) * renderer_bbox_image_up
        outside_bbox_image_1d_up = outside_bbox_image_up.view(bs,-1)
        inters_sum_up = torch.sum(outside_bbox_image_1d_up, 1)
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        if config.layout_flag:
            ...
            if(config.dp3d_vert_loss>0):
                ...
                mesh_loss_all = mesh_loss +  (sharp_loss_0 + sharp_loss_1) / len(end_points['deep3d_meshes']) * config.dp3d_sharp_loss_coef +  (smooth_loss_0 + smooth_loss_1) / len(end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef + config.dp3d_vert_loss * init_vert_loss
            else:
                mesh_loss_all = mesh_loss +  (sharp_loss_0 + sharp_loss_1) / 2.0 * config.dp3d_sharp_loss_coef +  (smooth_loss_0 + smooth_loss_1) / len(end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef
...
def evaluate_one_epoch(epoch, test_loader, model, criterion_obj, criterion_layout, config):
    ...
    for batch_idx, batch_data_label in tqdm(enumerate(test_loader)):
        ...
        if config.layout_flag:
            ...
            mesh_loss_all = mesh_loss +  (sharp_loss_0 + sharp_loss_1) / len(end_points['deep3d_meshes']) * config.dp3d_sharp_loss_coef + \
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = model.cuda()
    if(args.layout_flag):
        output_name = 'panocontextformer_0114_F2_phy'
    else:
