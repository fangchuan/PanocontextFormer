...
def physical_violation_loss(end_points, renderer_up, renderer_left ,image_size):
    ...
    for prefix in prefixes:
        ...
        start = time.time()
        renderer_bbox_image_up = renderer_up(batch_bbox_mesh)
        print("box render time {}".format(time.time() - start))
        
        renderer_bbox_image_up = renderer_bbox_image_up.view(bs,-1, image_size, image_size, 1)

        # renderer_bbox_image_left = renderer_left(batch_bbox_mesh)
        # renderer_bbox_image_left = renderer_bbox_image_left.view(bs,-1, image_size, image_size, 1)
        ...
        start = time.time()
        renderer_layout_image_up = renderer_up(textured_mesh).unsqueeze(1)
        print("layout render time {}".format(time.time() - start))
        # renderer_layout_image_left = renderer_left(textured_mesh).unsqueeze(1)

        outside_mask_up = 1 - renderer_layout_image_up
        outside_bbox_image_up = outside_mask_up * renderer_bbox_image_up
        outside_bbox_image_1d_up = outside_bbox_image_up.view(bs,256,-1)
        inters_sum_up = torch.sum(outside_bbox_image_1d_up, 2)

        # outside_mask_left = 1 - renderer_layout_image_left
        # renderer_layout_image_left = outside_mask_left * renderer_bbox_image_left
        # outside_bbox_image_1d_left = renderer_layout_image_left.view(bs,256,-1)
        # inters_sum_left = torch.sum(outside_bbox_image_1d_left, 2)

        obj_ass_mask = end_points[f'{prefix}objectness_label'].float()

        mask_and = torch.sum(torch.logical_and(renderer_layout_image_up>0.5, renderer_bbox_image_up>0.5).view(bs,256,-1),2)
        renderer_bbox_image_mask = torch.sum((renderer_bbox_image_up>0.5).view(bs,256,-1),2)
        inside_mask = (mask_and==renderer_bbox_image_mask)

        inv_mask_and = torch.sum(torch.logical_and(renderer_layout_image_up<0.5, renderer_bbox_image_up>0.5).view(bs,256,-1),2)
        ...
        physical_loss = torch.sum(inters_sum_up * final_mask)/(torch.sum(final_mask) + 1e-6)/image_size
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        if config.layout_flag:
            ...
            if(config.dp3d_vert_loss>0):
                ...
                mesh_loss_all = mesh_loss +  (sharp_loss_0 + sharp_loss_1) / 2.0 * config.dp3d_sharp_loss_coef + sharp_loss_2 * config.dp3d_sharp_loss_coef_last + \
                             (smooth_loss_0 + smooth_loss_1 + smooth_loss_2) / len(end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef + config.dp3d_vert_loss * init_vert_loss
            else:
                mesh_loss_all = mesh_loss +  (sharp_loss_0 + sharp_loss_1) / 2.0 * config.dp3d_sharp_loss_coef + sharp_loss_2 * config.dp3d_sharp_loss_coef_last + \
            ...
            if(config.phy_viol_delta>0):
                violation_loss, end_points = physical_violation_loss(end_points, renderer_up, renderer_left, image_size)
            else:
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = model.cuda()
    if(args.layout_flag):
        output_name = 'layout_mesh_detection_0111_test'
    else:
        output_name = 'detection_only_group_free'
