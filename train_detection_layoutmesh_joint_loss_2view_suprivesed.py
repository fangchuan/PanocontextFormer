...
def physical_violation_loss(end_points, renderer_up, renderer_left ,image_size):
    ...
    for prefix in prefixes:
        ...
        physical_loss = torch.sum(inters_sum_up * final_mask + inters_sum_left * final_mask)/(torch.sum(final_mask) + 1e-6)/image_size
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion_obj, criterion_layout, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        if config.layout_flag:
            ...
            init_vert_loss_coef = 2.0

            mesh_loss_all = mesh_loss +  (sharp_loss_1 + sharp_loss_2) / len(end_points['deep3d_meshes']) * config.dp3d_sharp_loss_coef + \
                         (smooth_loss_1 + smooth_loss_2) / len(end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef + init_vert_loss_coef * init_vert_loss
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = model.cuda()
    if(args.layout_flag):
        output_name = 'layoutmesh_detection_render_1211'
    else:
