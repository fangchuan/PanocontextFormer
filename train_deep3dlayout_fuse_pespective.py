...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, mesh_criterion, shape_criterion, smooth_criterion, vert_criterion, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        init_vert_loss = vert_criterion(end_points['deep3d_meshes'][0].verts_padded(),batch_data_label['initial_mesh_loc'])
        losses['init_vert_loss'] = init_vert_loss.item()
        ...
        total_loss = mesh_loss + \
             ...
             (smooth_loss_0 + smooth_loss_1)/len(end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef + config.dp3d_vert_loss * init_vert_loss
...
if __name__ == '__main__':
    ...
    model = Deep3DlayoutNetFuse(backbone='resnet18', decoder_type='rcnn_p2m_mhsa_pos_dual', full_size=True, hidden_dim=128, fuse_type=args.fuse_type)
    if(torch.cuda.is_available()):
        model = model.cuda()

    train_loader, test_loader, DATASET_CONFIG = get_loader(args)

    LOG_DIR = os.path.join(args.log_dir, 'deep3dlayout_global_0108_v386', f'{args.dataset}_{int(time.time())}')
    args.log_dir = LOG_DIR
    os.makedirs(args.log_dir, exist_ok=True)

    logger = setup_logger(output=args.log_dir, name="deep3dlayout_fuse")
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, mesh_criterion, sharp_criterion, smooth_criterion, vert_criterion, optimizer, scheduler, args)
