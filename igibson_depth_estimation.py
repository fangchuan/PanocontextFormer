...
def load_checkpoint(model, args):
    ...
    model.load_state_dict(update_model_state_dict)
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, depth_criterion, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        depth_loss = depth_criterion(batch_data_label['gt_depth'], end_points['pred_depth'])
        ...
        losses['depth_loss'] = depth_loss.item() + losses['depth_loss']

        # Accumulate statistics and print out

        if (batch_idx + 1) % 10 == 0:
            logger.info(f'Train: [{epoch}][{batch_idx + 1}/{len(train_loader)}]  ' + ''.join(
                [f'{key} {losses[key]/(batch_idx+1)} \t' for key in losses]))
...
def evaluate_one_epoch(epoch, test_loader, model, depth_criterion, config):
    ...
    for batch_idx, batch_data_label in enumerate(test_loader):
        ...
        if (batch_idx + 1) % 10 == 0:
            logger.info(f'Eval: [{epoch}][{batch_idx + 1}/{len(test_loader)}]  ' + ''.join(
                [f'{key} {depth_loss.item()} \t']))

        cur_iter = (epoch-1)*len(test_loader)+batch_idx

        k = 'Eval/depth_loss'
        tb_writer.add_scalar(k, depth_loss.item(), cur_iter)
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        args.checkpoint_path = "/mnt/workspace/code/Unifuse_self_supervised/pretrain/Matterport3D/model.pth"
        model = UniFuse (18, 512, 1024, "", 10.0, fusion_type='cee', se_in_fusion=True)
        model = load_checkpoint(model, args)
        model = model.cuda()
    else:
        args.checkpoint_path = "/Users/yuandong/Documents/Git_project_DAMO/UniFuse-Unidirectional-Fusion/UniFuse/pretrain/Matterport3D/model.pth"
        model = UniFuse(18, 512, 1024, "", 10.0, fusion_type='cee', se_in_fusion=True)
        model = load_checkpoint(model, args)
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, depth_criterion, optimizer, scheduler, args)
        ...
        if(epoch%args.val_freq==0):
            evaluate_one_epoch(epoch//args.val_freq, test_loader, model, depth_criterion, args)
