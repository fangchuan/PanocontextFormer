...
class E2P(nn.Module):
    def __init__(self, equ_size, out_dim, fov, radius=128, up_flip=True, gpu=True):
        ...
        # vis_points(reshape_xyz.transpose(1,0))
...
if __name__ == '__main__':
    ...
    e2p = E2P((512, 1024), 128, 160, gpu=False)
    base_dir = "/Users/yuandong/Desktop/test/"
    image_list = glob(os.path.join(base_dir+"*.png"))
    out_dir = "/Users/yuandong/Desktop/test/"
    for index,name in enumerate(image_list):
        print(index,len(image_list))
        image = cv2.imread(name)
        # image = cv2.resize(image,(1024,512),cv2.INTER_AREA)
        image = np.transpose(image,(2,0,1)).astype(np.float32)
        image_tensor = torch.tensor(image)
        image_tensor = torch.unsqueeze(image_tensor,0)
        [up, down] = e2p(image_tensor)

        up_image = np.array(up).squeeze()
        up_image = np.transpose(up_image,(1,2,0))/255.

        down_image = np.array(down).squeeze()
        down_image = np.transpose(down_image,(1,2,0))/255.
        split_name = os.path.split(name)[-1][:-4]
        # plt.imshow(down_image[:,:,(2,1,0)])
        # plt.show()
        cv2.imwrite(out_dir+split_name+"_down.jpg",down_image*255)
