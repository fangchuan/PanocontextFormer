...
class Deep3DlayoutNet(nn.Module):
    ...
    def __init__(self, backbone, decoder_type='conv', full_size=True, hidden_dim=128, num_stages=2):
        ...
        from module.deep3dlayout.global_featrue import global_feature_project2
        self.get_global_featrue = global_feature_project2(hidden_dim)
    ...
    def forward(self, inputs):
        ...
        global_feature = self.get_global_featrue(conv_list)

        loacl_feature = self.slicing_module(conv_list, x.shape[3])

        output_dict = self.p2m(loacl_feature, global_feature, subdivide=self.subdivide, reshaped_fh=self.reshape_fh)
...
def counter():
    ...
    net = Deep3DlayoutNet(backbone='resnet18', decoder_type='rcnn_p2m_mhsa_pos_dual', full_size=True, hidden_dim=128)
