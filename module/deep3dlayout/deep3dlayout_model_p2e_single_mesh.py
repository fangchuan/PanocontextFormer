...
from module.deep3dlayout.gcn_single_mesh import MeshRefinementHead
...
class Slicing(nn.Module):
    ...
    def forward(self, x, out_w):
        ...
        if ((x.shape[3] != out_w) and self.interpolate):
            ...
            x = F.interpolate(x, size=(x.shape[2], out_w + 2 * factor), mode='bilinear',
                              align_corners=False)  ####NB interpolating only W
...
class SplittedMultiSlicing(nn.Module):
    def __init__(self, c1, c2, c3, c4, out_scale=8, backbone='resnet18', interpolate_feats=False, reshape_fh=True):
        ...
        self.slc_lst = nn.ModuleList([
            Slicing(c1, c1 // out_scale, encoder_type=backbone, interpolate=interpolate_feats),
            ##256->32 resnet50 ##64->8 resnet18
...
####Deep3DLayout model
class Deep3DlayoutNetFuse(nn.Module):
    ...
    def __init__(self, backbone, decoder_type='conv', full_size=True, hidden_dim=288, num_stages=2, fuse_type='biproj'):
        super(Deep3DlayoutNetFuse, self).__init__()
        ...
        if (backbone == 'resnet18' or backbone == 'resnet50'):
            ...
            with torch.no_grad():
                ...
                c1, c2, c3, c4 = [b.shape[1] for b in
                                  self.feature_extractor(dummy)]  ###NB depend by resnet layers depth
        ...
        self.slicing_module = SplittedMultiSlicing(c1, c2, c3, c4, self.out_scale, backbone=self.backbone,
                                                   interpolate_feats=False, reshape_fh=self.reshape_fh)

        lfeats_dim = c1 // self.out_scale + c2 // self.out_scale + c3 // self.out_scale + c4 // self.out_scale

        self.p2m = MeshRefinementHead(input_channels=lfeats_dim, hidden_dim=hidden_dim, stage_depth=6, use_mhsa=True,
                                      num_stages=num_stages, ico_sphere_level=3, use_pos_encoding=True)
        ...
        self.grid_map_128 = nn.Parameter(get_p2e_grid_sample(self.p_image_size, self.fp_fov, 128),
                                         requires_grad=False).to('cuda' if torch.cuda.is_available() else 'cpu')
        self.grid_map_64 = nn.Parameter(get_p2e_grid_sample(self.p_image_size, self.fp_fov, 64),
                                        requires_grad=False).to('cuda' if torch.cuda.is_available() else 'cpu')
        self.grid_map_32 = nn.Parameter(get_p2e_grid_sample(self.p_image_size, self.fp_fov, 32),
                                        requires_grad=False).to('cuda' if torch.cuda.is_available() else 'cpu')
        self.grid_map_16 = nn.Parameter(get_p2e_grid_sample(self.p_image_size, self.fp_fov, 16),
                                        requires_grad=False).to('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    def forward(self, inputs):
        ...
        output_dict = self.p2m(feature, meshes=None, subdivide=self.subdivide, reshaped_fh=self.reshape_fh)
        output_dict['gloabl_featrue'] = conv_list[-1]
...
def counter():
    ...
    net = Deep3DlayoutNetFuse('resnet18').to(device)
