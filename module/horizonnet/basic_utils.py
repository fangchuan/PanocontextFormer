    ...
    for k, t in key_type.items():
        ...
        if not (isinstance(new_value, dict) or if_force_list == True):
            if t == torch.Tensor:
                ...
                if(k == "x" or k == "y"):
                    max_len = 0
                    for _ in range(len(new_value)):
                        max_len = max(max_len,new_value[_].shape[0])
                    for _ in range(len(new_value)):
                        last_num = new_value[_][-1]
                        new_value_i = np.ones(max_len,dtype="int")*last_num
                        new_value_i[:new_value[_].shape[0]] = new_value[_]
                        new_value[_] = new_value_i
