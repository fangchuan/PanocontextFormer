...
import cv2
from module.layout.misc import panostretch
...
class PanoCorBonDataset(data.Dataset):
    ...
    def _check_dataset(self):
        for fname in self.txt_fnames:
            ...
            # if not os.path.isfile(os.path.join(self.cor_dir, fname)):
            #     mv_command1 = "mv "+ os.path.join(self.img_dir, fname[:-3]+"jpg")+ " /Users/yuandong/Desktop/0712新训练数据集/onlinedata_0806/nojson_img/"
            #     mv_command2 = "mv " + os.path.join(self.img_dir, fname[:-3] + "png") + " /Users/yuandong/Desktop/0712新训练数据集/onlinedata_0806/nojson_img/"
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.img_dir,
                                self.img_fnames[idx])
        # img = np.array(Image.open(img_path), np.float32)[..., :3]
        ...
        with open(os.path.join(self.cor_dir,
            ...
            for _ in range(cor.shape[0]//2):
                curr_index = _*2
                if cor_flag:
                    cv2.circle(img, (int(cor[curr_index, 0]), int(cor[curr_index,1])), 2, (255,0,  0),thickness=2)
                    cv2.circle(img, (int(cor[curr_index+1, 0]), int(cor[curr_index+1, 1])), 2, (255,0,  0), thickness=2)
                    p1 = (int(cor[curr_index, 0]), int(cor[curr_index,1]))
                    p2 = (int(cor[curr_index+1, 0]), int(cor[curr_index+1, 1]))
                    cv2.line(img,p1,p2, (0,255,  0),thickness=2)
            img = img/ 255.
            # plt.imshow(img)
...
def visualize_a_data(x, y_bon, y_cor):
    ...
    color = [255,0,0]
    color2 = [0,255,0]
    img_bon[y_bon[0], np.arange(len(y_bon[0])),:] = color2
    img_bon[y_bon[1], np.arange(len(y_bon[1])),:] = color
    img_bon[y_bon[0]+1, np.arange(len(y_bon[0])), :] = color2
    img_bon[y_bon[1]+1, np.arange(len(y_bon[1])), :] = color
    img_bon[y_bon[0]-1, np.arange(len(y_bon[0])), :] = color2
    img_bon[y_bon[1]-1, np.arange(len(y_bon[1])), :] = color
    # img_bon[y_bon[0]+2, np.arange(len(y_bon[0])), :] = color
    # img_bon[y_bon[1]+2, np.arange(len(y_bon[1])), :] = color
    # img_bon[y_bon[0]-2, np.arange(len(y_bon[0])), :] = color
    # img_bon[y_bon[1]-2, np.arange(len(y_bon[1])), :] = color

    # return np.concatenate([img_pad, img_bon], 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_dir', default="/Users/yuandong/Desktop/test/")
    parser.add_argument('--out_dir', default='/Users/yuandong/Desktop/test/visualization')
    parser.add_argument('--ignore_cor', default=False)
    ...
    print("start from : {}".format(args.start))
    for index in range(len(dataset)):
        print(index)
        if index<args.start:
            continue
        if args.ith >= 0:
            to_visualize = [dataset[index]]
        else:
