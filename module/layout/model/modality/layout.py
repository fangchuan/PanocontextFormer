...
from module.layout.misc import panostretch, post_proc
...
class LayoutEstimator(nn.Module):
    def __init__(self, emb_dim, bon_weight=1., cor_weight=1., bon_loss='l1', cor_loss='bce', bon_scale=1.,
                 init_weight=0.1, dropout=0., oneconv=True, last_ks=1, last_bias=True, sigmod_normlize=False,
                 sample_weight=False, H=512, W=1024, post_force_cuboid=False):
    ...
    def forward(self, x_emb):
        ...
        return pred_bon, pred_cor

    def infer(self, x_emb):
        pred_bon, pred_cor = self(x_emb)
    ...
    def compute_losses(self, x_emb, batch):
        ...
        losses = {}
        if self.bon_loss == 'l1':
            losses['bon'] = F.l1_loss(pred_bon, gt_bon)
        elif self.bon_loss == 'l2':
            losses['bon'] = F.mse_loss(pred_bon, gt_bon)
        elif self.bon_loss == "3d_loss":
            # config variable
            ratio = batch['ratio']
            gt_lonlat = batch['gt_up_uv']
            pred_lonlat_up = torch.cat([batch['unit_lonlat'][:, :, 0:1], pred_bon[:, 0, :, None]], dim=-1)
            pred_lonlat_down = torch.cat([batch['unit_lonlat'][:, :, 0:1], pred_bon[:, 1, :, None]], dim=-1)
            gt_lonlat_up = torch.cat([batch['unit_lonlat'][:, :, 0:1], gt_bon[:, 0, :, None]], dim=-1)
            gt_lonlat_down = torch.cat([batch['unit_lonlat'][:, :, 0:1], gt_bon[:, 1, :, None]], dim=-1)
            self.loss_3d.setGrid(batch['unit_xyz'][0, ...][None, None, ...])
            losses['bon'] = self.loss_3d(pred_lonlat_up, pred_lonlat_down, gt_lonlat, ratio, gt_lonlat_up, gt_lonlat_down, self.sample_weight)

        else:
            raise NotImplementedError

        if self.cor_loss == 'bce':
            losses['cor'] = F.binary_cross_entropy_with_logits(pred_cor, gt_cor)
        ...
        with torch.no_grad():
            losses['bon.mae'] = F.l1_loss(pred_bon, gt_bon) / self.bon_scale
            losses['cor.mae'] = F.l1_loss(pred_cor.sigmoid(), gt_cor)
        return losses
