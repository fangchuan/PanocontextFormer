...
def physical_violation_loss(end_points, renderer, image_size):
    ...
    for prefix in prefixes:
        ...
        pred_mesh_expand = pred_mesh.extend(bbox_num)
        # print("pred_mesh_expand = ", pred_mesh_expand.verts_padded().shape)
        # exit()
        ...
        # np.save("render_dump/renderer_layout_image.npy",renderer_layout_image.detach().cpu().numpy())
        # np.save("render_dump/renderer_bbox_image.npy",renderer_bbox_image.detach().cpu().numpy())
        # exit()
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion_obj, criterion_layout, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        if config.layout_flag:
            ...
            print('violation_loss Elapsed time', time.time() - start)
    ...
    for batch_idx, batch_data_label in tqdm(enumerate(test_loader)):
        ...
        if config.layout_flag:
            ...
            if (batch_idx<5):
                ...
                save_obj(output_filepath+"_corss.obj", end_points['deep3d_meshes'][2].cpu().detach().verts_padded()[0,:,:], end_points['deep3d_meshes'][1].cpu().detach().faces_padded()[0,:,:])
        else:
            end_points['total_loss'] = end_points['object_loss']
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = model.cuda()
    if(args.layout_flag):
        output_name = 'layoutmesh_detection_render_test'
    else:
