...
def physical_violation_loss_cube(end_points):
    # prefixes = ['last_', 'proposal_'] + [f'{i}head_' for i in range(5)]
    prefixes = ['last_']
    
    # render layout_mesh
    pred_mesh = end_points['deep3d_meshes'][-1]
    pred_mesh_padded = pred_mesh.verts_padded()
    
    layout_max_pt = torch.max(pred_mesh_padded, dim=1)[0].unsqueeze(1).repeat(1,256,1)
    layout_min_pt = torch.min(pred_mesh_padded, dim=1)[0].unsqueeze(1).repeat(1,256,1)
    ...
    for prefix_idx, prefix in enumerate(prefixes):
        ...
        bbox_verts_packed = batch_bbox_mesh.verts_padded()
        bbox_verts_padded = bbox_verts_packed.view(bs, 256, 8, 3)
        bbox_max_pt = torch.max(bbox_verts_padded, dim=2)[0]
        bbox_min_pt = torch.min(bbox_verts_padded, dim=2)[0]

        violation = torch.nn.functional.relu(bbox_max_pt - layout_max_pt) + \
                    torch.nn.functional.relu(layout_min_pt - bbox_min_pt)  # b x 3

        final_mask = obj_ass_mask * (1.0 - obj_outside_label)

        violation_loss = l1_loss(violation)
        violation_loss = torch.sum(violation_loss * final_mask.unsqueeze(2)) / (torch.sum(final_mask) + 1e-6)

        end_points[f'{prefix}physical_cube_loss'] = violation_loss
        if(prefix_idx==0):
            total_violation_loss = violation_loss
        else:
            total_violation_loss = violation_loss + total_violation_loss


    return total_violation_loss/len(prefixes), end_points
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = model.cuda()
    if(args.layout_flag):
        output_name = 'contextformer_0114_F_cube_phy'
    else:
