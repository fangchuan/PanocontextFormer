...
def get_merged_bs_meshes3(all_verts, all_edges, obj_ass_label, outside_label):
    bs = obj_ass_label.shape[0]
    return_meshes = []
    for bs_idx in range(bs):
        merged_mesh_verts_list = []
        merged_mesh_edges_list = []
        for i in range(256):
            bbox_idx = 256 * bs_idx + i
            if (obj_ass_label[bs_idx, i] < 0.5 or outside_label[bs_idx, i] > 0.5): continue
            merged_mesh_verts_list.append(all_verts[bbox_idx, :, :])
            merged_mesh_edges_list.append(all_edges[bbox_idx, :, :] + len(merged_mesh_edges_list) * 8)
        if (len(merged_mesh_verts_list) == 0):
            logger.info("Warning: empty bbox!")
            standard_bbox = get_standard_bbox()
            merged_mesh_verts = standard_bbox.verts_packed() * 0.01
            merged_mesh_edges = standard_bbox.faces_packed()
        else:
            merged_mesh_verts = torch.cat(merged_mesh_verts_list, 0)
            merged_mesh_edges = torch.cat(merged_mesh_edges_list, 0)
        merged_textured_mesh = Meshes(
            verts=[merged_mesh_verts.to('cuda')],
            faces=[merged_mesh_edges.to('cuda')],
        )
        return_meshes.append(merged_textured_mesh)
    return pytorch3d.structures.join_meshes_as_batch(return_meshes)


def physical_violation_loss(end_points, renderer, render2mesh = False):
    # prefixes = ['last_']
    prefixes = ['last_', 'proposal_'] + [f'{i}head_' for i in range(5)]

    # render layout_mesh
    renderer_layout_image = renderer(end_points['deep3d_meshes'][-1])[..., 3]
    if(render2mesh):
        init_renderer_layout_image = renderer(end_points['deep3d_meshes'][0])[..., 3]
    ...
    for prefix_idx, prefix in enumerate(prefixes):
        ...
        obj_outside_label = end_points[f'{prefix}outside_label'].float()
        ...
        pred_heading_residual = torch.gather(pred_heading_residual, 2,
                                             pred_heading_class_argmax.unsqueeze(-1))  # B,num_proposal,1
        pred_heading_class = softargmax1d(heading_scores.contiguous().view(-1, Num_heading_bin)).view(bs, -1).unsqueeze(
            -1)
        ...
        batch_bbox_mesh_verts = (batch_bbox_mesh.verts_padded()).repeat(bbox_num, 1, 1)

        # apply_size
        all_size = pred_size.contiguous().view(-1, 3).unsqueeze(1).repeat(1,8,1)
        batch_bbox_mesh_verts = all_size * batch_bbox_mesh_verts
        ...
        updated_verts = transform.transform_points(batch_bbox_mesh_verts)
        all_edges = batch_bbox_mesh.faces_padded().repeat(bbox_num, 1, 1)

        # merge_good_bbox_mesh
        merged_batch_bbox_meshes = get_merged_bs_meshes3(updated_verts, all_edges, obj_ass_mask,obj_outside_label)

        renderer_bbox_image = renderer(merged_batch_bbox_meshes)[..., 3]

        if (render2mesh):
            outside_bbox_image = ((1.0 - renderer_layout_image) * (renderer_bbox_image) + (1.0 - init_renderer_layout_image) * (renderer_bbox_image))/2.0
        else:
            outside_bbox_image = (1.0 - renderer_layout_image) * (renderer_bbox_image)
        outside_bbox_image_1d = outside_bbox_image.view(bs, -1)
        inters_sum_up = torch.sum(outside_bbox_image_1d, 1)

        final_mask = (1 - obj_outside_label) * obj_ass_mask
        current_physical_loss = torch.sum(inters_sum_up) / (torch.sum(final_mask) + 1e-6)
        if (prefix_idx == 0):
            physical_loss = current_physical_loss
        else:
            physical_loss = physical_loss + current_physical_loss
        end_points[f'{prefix}physical_loss'] = current_physical_loss

    return physical_loss / len(prefixes), end_points
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion_obj, criterion_layout, optimizer, scheduler,
                    config):
    ...
    shader_up = SoftSilhouetteShader(blend_params=blend_params)
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        if config.layout_flag:
            ...
            if (config.dp3d_vert_loss > 0):
                ...
                mesh_loss_all = mesh_loss + (sharp_loss_0 + sharp_loss_1) / len(
                    end_points['deep3d_meshes']) * config.dp3d_sharp_loss_coef + (smooth_loss_0 + smooth_loss_1) / len(
                    end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef + config.dp3d_vert_loss * init_vert_loss
            else:
                mesh_loss_all = mesh_loss + (sharp_loss_0 + sharp_loss_1) / 2.0 * config.dp3d_sharp_loss_coef + (
                        smooth_loss_0 + smooth_loss_1) / len(
                    end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef

            end_points['layout_loss'] = mesh_loss_all
            if (config.phy_viol_delta > 0):
                violation_loss, end_points = physical_violation_loss(end_points, renderer_up, False)
            else:
                violation_loss = torch.tensor(0)

            end_points['total_loss'] = end_points['object_loss'] + config.mesh_delta * mesh_loss_all + violation_loss * config.phy_viol_delta
...
def evaluate_one_epoch(epoch, test_loader, model, criterion_obj, criterion_layout, config):
    ...
    for batch_idx, batch_data_label in tqdm(enumerate(test_loader)):
        ...
        if config.layout_flag:
            ...
            mesh_loss_all = mesh_loss + (sharp_loss_0 + sharp_loss_1) / len(
                end_points['deep3d_meshes']) * config.dp3d_sharp_loss_coef + \
                            (smooth_loss_0 + smooth_loss_1) / len(
                end_points['deep3d_meshes']) * config.dp3d_smooth_loss_coef
            ...
            if (batch_idx < 5):
                save_obj(output_filepath + "_initial.obj",
                         end_points['deep3d_meshes'][0].cpu().detach().verts_padded()[0, :, :],
...
def get_model(config, DATASET_CONFIG):
    from models.detector_layout_mesh_joint import GroupFreeDetectorDeep3D
...
if __name__ == '__main__':
    ...
    if (torch.cuda.is_available()):
        model = model.cuda()
    if (args.layout_flag):
        output_name = 'panocontextformer_0115_F2_phy_alllayer'
    else:
        output_name = 'detection_only_0110_joint'
    ...
    param_dicts = [
        {"params": [p for n, p in model.named_parameters() if
                    "decoder" not in n and p.requires_grad and "layout_estimation" not in n]},
        {
            "params": [p for n, p in model.named_parameters() if
                       "decoder" in n and p.requires_grad and "layout_estimation" not in n],
            "lr": args.decoder_learning_rate,
        },
        {
            "params": [p for n, p in model.named_parameters() if
                       "decoder" not in n and p.requires_grad and "layout_estimation" in n],
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion_obj, criterion_layout, optimizer,
                        scheduler, args)
