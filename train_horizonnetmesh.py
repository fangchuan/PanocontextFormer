...
from module.horizonnet.layout_estimation import HorizonNetMesh
import torch.nn.functional as F
from module.deep3dlayout.mesh_loss import MeshLoss
from module.deep3dlayout.custom_losses.losses import L_sharp_loss
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion, mesh_criterion, shape_criterion, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        image_loss = criterion(end_points, batch_data_label, prefixe='initial_')
        gt_meshes = (batch_data_label['points_gt'],batch_data_label['normals_gt'])

        mesh_loss, losses = mesh_criterion(meshes_pred = end_points['pred_mesh'], meshes_gt = gt_meshes)

        # compute sharp-loss
        sharp_loss = shape_criterion(meshes_pred = end_points['pred_mesh'], meshes_gt = batch_data_label)
        losses['sharp_loss'] = sharp_loss.item()

        total_loss = image_loss['initial_'+'layout_loss'] + mesh_loss + sharp_loss
        ...
        for key in image_loss:
            losses[key] = image_loss[key].item()
...
def evaluate_one_epoch(epoch, test_loader, model,  criterion, mesh_criterion, shape_criterion, config):
...
if __name__ == '__main__':
    ...
    if(torch.cuda.is_available()):
        model = HorizonNetMesh(cfg=args, pretrain_ckpt='/mnt/workspace/code/DeepPanoContext/pretratin_weight/HorizonNet/resnet50_rnn__st3d.pth')
        model = model.cuda()
    else:
        model = HorizonNetMesh(cfg=args, pretrain_ckpt='/Users/yuandong/Documents/Git_project_DAMO/DeepPanoContext_PAI/pretratin_weight/HorizonNet/resnet50_rnn__st3d.pth')

    train_loader, test_loader, DATASET_CONFIG = get_loader(args)

    LOG_DIR = os.path.join(args.log_dir, 'layout_estimation',
    ...
    logger = setup_logger(output=args.log_dir, name="layout_estimation")
    ...
    mesh_criterion = MeshLoss(chamfer_weight=1.0, normal_weight=0.1,
                              edge_weight=0.1, gt_num_samples=5000,
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion, mesh_criterion, sharp_criterion, optimizer, scheduler, args)
        ...
        if(epoch%args.val_freq==0):
            evaluate_one_epoch(epoch//args.val_freq, test_loader, model, criterion, mesh_criterion, sharp_criterion, args)
            if(epoch%50):
